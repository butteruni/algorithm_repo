...
int main()
{
    ...
    {
        // Line a = Line(ans.p[i],ans.p[i + 1]);
        Point a = ans.p[i + 1] - ans.p[i];
        while((a ^ (ans.p[j] - ans.p[i])) < (a ^ (ans.p[j + 1] - ans.p[i])))
        {
            j++;
            if(j == ans.n)
                j = 0;
        }
        long long cur_ans = max((ans.p[j] - ans.p[i]).len2(),(ans.p[j + 1] - ans.p[i]).len2());
        cur_ans = max(cur_ans,a.len2());
        mx = max(mx,cur_ans);
        // cout << cur_ans << '\n';
    }
    // cout << mx << '\n';
