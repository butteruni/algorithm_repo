...
unsigned main()
{
    ...
    cin >> n;
    n++;
    vector<Z>fiv(n + 10),inv(n + 10);
    fiv[0] = fiv[1] = inv[0] = inv[1] = 1;
    for(int i = 2;i <= n;++i)
    {
        inv[i] = mod - (mod / i) * inv[mod % i];
        fiv[i] = fiv[i - 1] * inv[i].getMod();
    }
    g2[0] = 1;
    for(int i = 1;i < n;++i)
    {
        ll temp = power(2,1ll * i * (i - 1) / 2 % (mod - 1));
        g1[i] = temp * fiv[i - 1].getMod();
        g2[i] = temp * fiv[i].getMod()
    }
    g2 = 
}
