...
int main() {
    ...
    std::vector<Z> a(n + m), b(n + m), c(n), d(n + m);
    ...
    auto solve1 = [&](auto self, int l, int r) {
        std::array<Poly<P>, 5> res;
        if (r - l == 1) {
            res[0] = {1, -(1 - p) * b[l]};
            res[1] = {0, p * a[l]};
            res[2] = {d[l]};
        ...
        res[0] = fl[0] * fr[0];
        res[1] = fl[1] * fr[1];
        res[2] = fl[2] * fr[1] + fr[2] * fl[0];
        res[3] = fl[3] * fr[0] + fr[3] * fl[1];
        res[4] = fl[4] * fr[0] + fr[4] * fl[0] + fl[2] * fr[3];
        return res;
    };
    auto res = solve1(solve1, 0, n + m);
    
    auto A = (res[4] * res[0].inv(n + 1)).trunc(n + 1);
    
    std::vector<Z> ans(n);
    
    auto solve2 = [&](auto self, int l, int r, Poly<P> a, int k) {
        if (k < l) {
            a = a.shift(k - l);
            k = l;
        }
        ...
        int m = (l + r) / 2;
        auto v = self(self, l, m, a, k);
        v *= self(self, m, r, a * v, k);
        return v;
    };
    solve2(solve2, 0, n, A, 0);
